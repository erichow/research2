...
同一份 JS（`shared.js`），agent 根据不同的设计稿生成截然不同的 HTML——终端模拟器、移动端气泡……交互逻辑完全不用改。所有交互由 `data-ref` 和 `data-action` 驱动，零依赖，在浏览器直接打开就能跑。
...
> **AI 聊天助手** — 用户能创建、切换、删除、搜索会话；能发送消息并查看对话历史；AI 回复以打字机效果逐字显示；空会话显示欢迎界面和快捷提问入口；支持亮色/暗色模式切换、多模型选择、文件上传（模拟）。

注意：PM 只描述了**用户能做什么**，没说一句"左侧右侧""全屏布局"——那是设计师的工作。
...
|--------|-------------|
| `sessionList` | 会话列表的容器 |
| `chatMessages` | 对话消息的展示区 |
| `chatInput` | 文本输入框 |
| `searchInput` | 会话搜索输入框 |
| `typingIndicator` | AI 流式回复时的打字提示 |
| `welcomeScreen` | 欢迎界面（空会话时展示） |
...
设计师给出视觉稿——布局、配色、交互样式。设计稿可以有多个版本（甚至来自不同设计师），但不管哪版，语义层都不需要动。

> 设计稿具体是什么样子不重要，重要的是 step 1 定义的 `data-ref` / `data-action` 是稳定的。设计师可以自由发挥——终端模拟器、移动端气泡、桌面双栏——全都由这份语义契约保证交互一致。
...
两个 Demo（终端模拟器、移动端气泡）共用同一份 `shared.js`，就是最好的证明。
